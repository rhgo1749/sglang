...
CONTAINER="${CONTAINER:-sglang-qwen38-vanilla-pp3-capacity}"
PORT="${PORT:-30000}"
...
cleanup_old() {
  docker rm -f sglang-qwen38-test sglang-qwen38-vanilla-pp3 "$CONTAINER" >/dev/null 2>&1 || true
}

dump_logs() {
  echo '=== IMPORTANT LOGS ==='
  docker logs "$CONTAINER" 2>&1 | \
    grep -Ei 'pipeline|pp.rank|KV Cache|#tokens|max_total_num_tokens|Memory pool|available_gpu_mem|Mamba|FP8|Traceback|AssertionError|RuntimeError|ValueError|CUDA error|out of memory|NCCL|exception' | \
    tail -600 || true
  echo '=== LOG TAIL ==='
  docker logs "$CONTAINER" 2>&1 | tail -250 || true
  echo '=== GPU STATE ==='
  nvidia-smi --query-gpu=index,name,memory.used,memory.total,utilization.gpu,pstate --format=csv || true
}

echo '============================================================'
echo ' QWEN3.8 PURE VANILLA PP3 CAPACITY SMOKE — MTP OFF'
echo ' Goal: determine whether PP3 alone can physically hold 3 x 262144 raw tokens'
...
for arg in '--pp-size' '--kv-cache-dtype' '--max-running-requests' '--context-length'; do
  grep -q -- "$arg" "$HELP" || { echo "ERROR: exact image lacks $arg"; exit 1; }
done
EXTRA_ARGS=()
if grep -q -- '--disable-cuda-graph' "$HELP"; then
  EXTRA_ARGS+=(--disable-cuda-graph)
  echo 'capacity mode: CUDA graph disabled to measure the vanilla VRAM ceiling'
else
  echo 'capacity mode: image has no --disable-cuda-graph flag; using default graph behavior'
fi
...
echo '=== START PURE VANILLA PP3, MTP OFF ==='
docker run -d \
  --name "$CONTAINER" \
  --gpus '"device=0,2,1"' \
  --ipc=host \
...
'; then
  echo '❌ VANILLA PP3 (MTP OFF) DID NOT BOOT'
  dump_logs
  exit 1
fi

echo '✅ VANILLA PP3 (MTP OFF) BOOTED'
LOG="$(mktemp)"
docker logs --since "$STARTED" "$CONTAINER" >"$LOG" 2>&1 || true

echo '=== CAPACITY / TOPOLOGY ==='
grep -Ei 'pipeline|pp.rank|KV Cache|#tokens|max_total_num_tokens|Memory pool end|available_gpu_mem|Mamba Cache|FP8' "$LOG" | tail -400 || true
...
vals = []
patterns = [
    r'max_total_num_tokens[^0-9]*([0-9]+)',
    r'#tokens:\s*([0-9]+)',
    r'#tokens[^0-9]*([0-9]+)',
]
for pat in patterns:
    vals.extend(int(x) for x in re.findall(pat, text, flags=re.I))
# Preserve useful unique values; duplicate log lines across ranks are expected.
uniq = []
for v in vals:
    if v not in uniq:
        uniq.append(v)
...
echo '=== CHEAP SINGLE REQUEST ==='
python3 - <<'PY'
import json
json.dump({
  'input_ids': [1000] * 256,
  'sampling_params': {'temperature': 0, 'max_new_tokens': 16, 'ignore_eos': True},
}, open('/tmp/qwen38-vanilla-pp3-capacity-one.json','w'))
PY
HTTP="$(curl --max-time 120 -sS -o /tmp/qwen38-vanilla-pp3-capacity-one-response.json -w '%{http_code}' \
  "http://127.0.0.1:${PORT}/generate" -H 'Content-Type: application/json' \
  --data-binary @/tmp/qwen38-vanilla-pp3-capacity-one.json)"
echo "single_http=$HTTP"
python3 - <<'PY'
import json
try:
    d=json.load(open('/tmp/qwen38-vanilla-pp3-capacity-one-response.json'))
    m=d.get('meta_info',{})
...
echo '============================================================'
echo ' INTERPRETATION'
echo ' - If boot fails: vanilla Qwen3.8 PP3 itself is not viable in this image/config.'
echo " - If per-stage capacity >= ${REQUIRED}: PP3 solves raw 3x256K capacity; only PP+MTP support is missing."
echo " - If per-stage capacity <  ${REQUIRED}: even vanilla PP3 cannot hold three fully-resident 256K sessions on these GPUs."
echo ' - Do NOT sum PP-stage token counts: every PP stage must represent all live token positions for its own layers.'
echo '============================================================'
