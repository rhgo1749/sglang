...
def replace_once(text: str, old: str, new: str, label: str, path: pathlib.Path) -> tuple[str, bool]:
    if new in text:
        return text, False
    if old not in text:
...
def patch(path: pathlib.Path) -> bool:
    ...
    changed |= c
    text, c = replace_once(text, ARGS_OLD, ARGS_NEW, "private sidecar args", path)
    changed |= c
    text, c = replace_once(text, POOL_OLD, POOL_NEW, "sidecar page invariant", path)
    changed |= c
